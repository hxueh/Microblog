...
class LoginForm(FlaskForm):
    ...
    token = StringField('Two Factor Authentication Token, '
                        'leave it blank if not setup.')
...
class SignupForm(FlaskForm):
    ...
    repeat = PasswordField('Repeat Password', validators=[
        DataRequired(), Length(6, 128), EqualTo('password')])
    submit = SubmitField('Submit')
...
class ChangePasswordForm(FlaskForm):
    ...
    new_password = PasswordField('New Password', validators=[
        DataRequired(), Length(6, 128)])
    repeat_new_password = PasswordField('New Password', validators=[
...
class ResetPasswordForm(FlaskForm):
    password = PasswordField('New Password', validators=[
        DataRequired(), Length(6, 128)])
    repeat_password = PasswordField('Repet New Password', validators=[
        DataRequired(), Length(6, 128), EqualTo('password')])
    submit = SubmitField('Submit')
